...
describe("legacy Order hooks", () => { it("loads list and detail snapshots", async () => {
  const store = new MemoryCommerceStore([], { orders: [ORDER_A] }); const runtime = createCommerceServices({ dataSource: "mock" }, { store });
  ...
  const hook = renderHook(() => ({ list: useLegacyOrders({ workspaceId: "ws-a", legacyBusinessUnitId: "bu-a", branchId: "br-a" }), item: useLegacyOrder({ workspaceId: "ws-a", legacyBusinessUnitId: "bu-a", branchId: "br-a" }, ORDER_A.id) }), { wrapper });
