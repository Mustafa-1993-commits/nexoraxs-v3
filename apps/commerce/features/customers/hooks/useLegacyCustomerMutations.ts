...
export function useLegacyCustomerMutations(scope: LegacyCommerceBusinessUnitScope) {
  ...
  const create = useMutation({ mutationFn: (command: CreateLegacyCustomerCommand) => services.customersFacade.create(scope, command), onSuccess: async (record) => {
  ...
  const update = useMutation({ mutationFn: (command: UpdateLegacyCustomerCommand) => services.customersFacade.update(scope, command), onSuccess: async (record) => {
