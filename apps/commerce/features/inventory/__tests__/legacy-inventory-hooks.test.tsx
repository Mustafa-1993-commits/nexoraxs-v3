...
describe("legacy Inventory hook", () => { it("returns the Branch projection", async () => {
  ...
  const store = new MemoryCommerceStore([product]); const wrapper = ({ children }: { children: ReactNode }) => <CommerceServicesProvider config={{ dataSource: "mock" }} overrides={{ store }}>{children}</CommerceServicesProvider>;
