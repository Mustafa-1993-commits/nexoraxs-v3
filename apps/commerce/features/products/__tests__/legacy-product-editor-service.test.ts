...
import { MemoryCommerceStore, MockProductsRepository } from "@nexoraxs/sdk";
...
describe("LegacyProductEditorService", () => {
  ...
  it("coordinates Product persistence and the separate media port", async () => {
    const { media, service } = setup();
    const imageFile = { name: "product.png" } as File;

    const result = await service.create(scope, command, imageFile);

    expect(media.saveProductImage).toHaveBeenCalledWith({ productId: "p_editor", file: imageFile });
  ...
  it("rolls back a newly-created Product when media coordination fails", async () => {
    ...
    await expect(service.create(scope, command, { name: "bad.png" } as File)).rejects.toThrow(
  ...
  it("does not update Product state when edit media coordination fails", async () => {
    ...
    await expect(service.update(scope, created.id, { name: "Must not commit" }, { name: "bad.png" } as File))
