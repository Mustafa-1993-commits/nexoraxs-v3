...
describe("legacy Invoice hooks", () => { it("loads list and document-keyed detail snapshots", async () => {
  const store = new MemoryCommerceStore([], { invoices: [INVOICE_A] }); const wrapper = ({ children }: { children: ReactNode }) => <CommerceServicesProvider config={{ dataSource: "mock" }} overrides={{ store }}>{children}</CommerceServicesProvider>;
