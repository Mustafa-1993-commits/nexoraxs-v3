...
describe("Feature 053 AppProvider compatibility characterization", () => {
  it("removes legacy Customer writes after the repository cutover", () => {
    ...
    expect(source).toContain("services.customersFacade.subscribe");
  });
  ...
  it("keeps Inventory, Order, Invoice, and Return writes on explicit retained paths", () => {
    ...
    expect(source).toContain("writeCollection(STORAGE_KEYS.branchInventory");
    expect(source).toContain("writeCollection(STORAGE_KEYS.orders");
    expect(source).toContain("writeCollection(STORAGE_KEYS.invoices");
  });
});
